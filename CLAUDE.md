...
requests, twenty-five diffs, forty commit diffs, evicted least recently fetched
first. A detail on a heavily reviewed pull request is megabytes and a branch
...
outranks a cap. Evicting a detail drops its `staleFetch`, `staleTimeline` and
`stalePulse` marks with it, since nothing is owed about a pull request no longer
held; `rowSeq` stays, because that belongs to a section's rows rather than to
this cache. The sections themselves are not part of any of this: that slice is
one entry per configured tab holding at most `PRsLimit` rows, and it never grows.
