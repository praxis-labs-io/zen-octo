...
- **Scrolling the shortest distance is right in exactly one place: a keystroke inside a box.** Everywhere else a key that lands on a block is taking the reader somewhere, so the block goes to the top row. A character typed is not taking them anywhere, and hauling the page for it is the worse of the two wrongs. The caret needs no arithmetic of its own: the box is a fixed height and the textarea scrolls inside it, so a box in view is a caret in view.
