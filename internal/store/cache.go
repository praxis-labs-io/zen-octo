...
// cache is a bounded map of fetched values, ordered by when each was last
// fetched. The zero value is usable: put builds the maps.
...
func newCache[V any]() cache[V] {
	return cache[V]{held: make(map[string]V), seen: make(map[string]int)}
}
...
// put writes a value and stamps it as the most recently fetched.
...
// evict drops the least recently fetched entries until the cache is inside cap,
// answering with the keys it dropped so a caller can clear what sits beside them.
func (c *cache[V]) evict(cap int, wrote string, pinned func(string) bool) []string {
	if len(c.held) <= cap {
		return nil
	}

	// Never the key just written: it is the most recently fetched thing there
	// is, and where the rest are pinned it is the only candidate left.
	...
	// entries goes over its cap: a pin outranks a cap, and a drop cannot be undone.
