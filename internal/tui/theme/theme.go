// Package theme derives the UI's colors from what the terminal reports.
package theme
...
// Theme is one palette. Nil optional fields fall back through their accessors.
type Theme struct {
	// Syntax names the Chroma style code is highlighted with. Empty falls back to Chroma's default.
	Syntax string

	Text   color.Color
	Accent color.Color
	// Subtle is text a reader still reads; Muted is chrome they glance at.
	Subtle   color.Color
	Muted    color.Color
	...
	// Background is nil where the terminal's own should show through.
...
// Only the low eight: a terminal may leave 8 to 15 undeclared or collapsed onto 0 to 7.
const (
...
// Luma distances rather than ratios: a ratio toward a hue the reader chose is not a fixed step.
const (
	tintLift      = 16
	selectionLift = 12
)

// The floor stops a pale hue reading grey; the ceiling binds where a hue sits at the background's weight.
const (
	minHueLift = 0.14
	maxLift    = 0.5
)

// Terminal derives the theme from what the terminal reported. transparent
// withholds Background and nothing else.
...
// The one place a slot is blended: a slot's RGBA() is canonical, so the reported hue comes first.
...
// A named background can leave the reported foreground on its own side of the midpoint.
